...
def _make_backward_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0):
    ...
    if past_key_values_length > 0:
        mask = torch.cat(
            [torch.full((tgt_len, past_key_values_length), float("-inf")), mask], dim=-1)
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    # mask: 1 for not masked 0 for masked, so it needs to be inverted
        ...
        for idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                encoder_states = encoder_states + \
                    (hidden_states,)  # add input embedding layer
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if idx > 0:
                intermediate_logits = lm_head(
                    hidden_states)  # using h_n to predict p_n
                ...
                if self.training:
                    ...
                    if labels is not None:
                        ...
                        control = torch.bernoulli(
                            probability).unsqueeze(-1).to(labels.device)
                        predicted_embeds = control * label_embeds + \
                            (1 - control) * predicted_embeds
                else:
                    predicted_samples = intermediate_logits.argmax(dim=-1)
                    predicted_embeds = self.embed_tokens(predicted_samples)
                hidden_states = concat_head(
                    torch.cat([hidden_states, predicted_embeds], dim=-1))
    ...
    def forward(
        ...
        if labels is not None:
            ...
            for logits in all_intermediate_logits:
                ...
                intermediate_lm_losses.append(
                    loss_fct(inter_logits.view(-1, self.config.vocab_size), labels.view(-1)))
